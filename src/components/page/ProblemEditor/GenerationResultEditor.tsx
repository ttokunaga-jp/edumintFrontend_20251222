...
export interface GenerationResultEditorProps {
  exam: any;
...
export function GenerationResultEditor({ exam, onChange, canEdit = true }: GenerationResultEditorProps) {
  ...
  const handleQuestionKeywordAdd = (qIdx: number, keyword: string) => {
    updateQuestion(qIdx, (question) => {
      ...
      keywords.push({ id: undefined, keyword });
  ...
  const handleSubQuestionFormatChange = (
    qIdx: number,
    sqIdx: number,
          ...
          {canEdit ? (
            <>
              <QuestionSectionEdit
                ...
                // viewMode is omitted (defaulting to 'full')
              />

              <Button variant="outline" size="sm" onClick={() => addSubQuestion(qIdx)}>
                <Plus /> 小問を追加
              </Button>
            </>
          ) : (
