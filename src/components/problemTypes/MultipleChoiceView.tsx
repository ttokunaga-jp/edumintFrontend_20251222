...
export default function MultipleChoiceView(props: ProblemTypeViewProps) {
  ...
  const renderOptionContent = (text: string) => {
    ...
    if (looksLikeLatex) return <LatexBlock content={text} displayMode={false} />;
        ...
        {options.map((opt, idx) => (
          <div key={opt.id}>
            <div style={{ display: 'flex', gap: '0.75rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{String.fromCharCode(65 + idx)}</div>
              <div>{renderOptionContent(opt.content)}</div>
              {showAnswer && opt.isCorrect && <div>正解</div>}
            </div>
          </div>
        ))}
      </div>
