...
import { useJobStore } from '../stores/jobStore'; export const useWebSocket = (jobId: string | null) => { const ws = useRef<WebSocket | null>(null); const { updatePhase, setError } = useJobStore(); useEffect(() => {
    if (!jobId) return;
    // Avoid opening a real WebSocket in environments without a WebSocket implementation (tests / some CI)
