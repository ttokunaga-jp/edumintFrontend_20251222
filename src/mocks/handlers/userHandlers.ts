...
export const userHandlers = [
  ...
  http.get(withBase('/user/:userId/problems'), ({ params, request }) => {
    ...
    const userExams = mockExams.filter((exam) => exam.userId === params.userId);
  ...
  http.get(withBase('/wallet/balance'), () => HttpResponse.json(mockWalletBalance)),
  http.post(withBase('/wallet/withdraw'), async ({ request }) => {
    const body = (await request.json().catch(() => ({}))) as Record<string, unknown>;
    const amount = typeof body.amount === 'number' ? body.amount : Number(body.amount ?? 0);
    if (!Number.isFinite(amount) || amount <= 0) {
      return HttpResponse.json({ success: false, message: 'Invalid amount' }, { status: 400 });
    }

    return HttpResponse.json({ success: true, message: 'Withdrawal request accepted' });
  }),];