...
type Params = {
  ...
  // structure_review → structure_detecting を許容するか（ユーザー操作のみ）
  allowStructureReanalysis?: boolean;
  // structure_detecting から structure_review を挟まずに次工程へ進むことを許容するか（自動確定/スキップ時）
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (allowStructureSkip && status.currentStep === 'structure_review') {
          normalizedStatus = {
            ...status,
        ...
        if (!isValid) {
          console.warn(`[GenerationStatus] Invalid transition observed (applying anyway): ${stateRef.current.currentStep} -> ${normalizedStatus.currentStep}`);
        }
